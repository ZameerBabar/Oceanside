...
import { db, auth } from '@/app/firebaseConfig'; // Make sure 'auth' is exported from your config
...
const ShiftNotesSystem = () => {
  ...
  const [formData, setFormData] = useState({
    location: 'Ormond',
    shiftType: 'AM',
    role: '', // Dynamically set from user data
    staffName: '', // Dynamically set from user data but can be edited
    notes: '',
    tags: []
  });
  ...
  // 1. Listen for authentication state changes and fetch user data
  useEffect(() => {
  ...
  // 2. Fetch the current user's notes once user data is available
  useEffect(() => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const noteData = {
        ...
        userAvatar: userData.avatar || null // Use avatar from Firestore if available
      };

      // Store in the 'notes' subcollection under the current user's document
      const notesRef = collection(db, 'users', currentUser.uid, 'notes');
  ...
  if (userLoading || !currentUser || !userData) {
    return (
      <div className="text-center p-8 bg-white ">
        ...
        <h2 className="text-xl font-semibold text-gray-800">Loading your data...</h2>
      </div>
    );
  }
