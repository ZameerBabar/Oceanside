...
import { db, auth } from "@/app/firebaseConfig"; // Ensure 'auth' is exported here
import SignaturePad from 'react-signature-canvas';

const sections = [
  { title: "Welcome", key: "welcome" },
  { title: "Mission Statement", key: "missionStatement" },
  { title: "Employment Policies", key: "employmentPolicies" },
  { title: "Team Member Responsibilities", key: "teamMemberResponsibilities" },
  { title: "Payroll & Timekeeping", key: "payrollAndTimekeeping" },
...
export default function EmployeeManual() {
    ...
    const fetchManualData = async () => {
      try {
        ...
        if (user) {
          ...
          if (userDocSnap.exists() && userDocSnap.data().acknowledgement) {
            setIsAcknowledged(true);
            setFullName(userDocSnap.data().acknowledgement.fullName || '');
            setEmail(userDocSnap.data().acknowledgement.email || '');
            setSignatureDataUrl(userDocSnap.data().acknowledgement.signatureData || '');
          }
        }
        
        const docRef = doc(db, "Employee Manual", "data");
  ...
  const handlePracticalSubmit = async (e) => {
    ...
    // Check if a signature has been provided using signaturePadRef
    ...
    try {
      ...
      alert(`Thank you, ${fullName}, for acknowledging the handbook! Your data has been saved.`);
      setIsAcknowledged(true);
  ...
  if (loading) {
    return (
      <div className="flex h-screen items-center justify-center bg-gradient-to-br from-green-100 to-green-600">
        <p className="text-xl text-green-800">Loading employee manual...</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-screen">
      {/* Sidebar */}
      <aside className="w-64 p-4 flex flex-col text-white bg-gradient-to-br from-green-200 to-green-800">
              ...
              <form onSubmit={handlePracticalSubmit} className="space-y-4">
                ...
                <div className="flex items-start">
                  ...
                  <label className="ml-2 block text-sm text-gray-700">
                    I acknowledge that I have read, understood, and agreed to abide by the policies and procedures outlined in the Oceanside Beach Bar & Grill Employee Handbook.
                  </label>
                </div>
